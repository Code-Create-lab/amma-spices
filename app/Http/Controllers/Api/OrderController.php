...
class OrderController extends Controller
{
    ...
    public function razorpay_order(Request $request)
    {
        ...
        $db = DB::table('orders')
            ...
            ->first();
        $secret = Setting::where('name', 'razorpay_secret_key')->select('value')->first();
        $key_id = Setting::where('name', 'razorpay_key_id')->select('value')->first();
        $api = new Api($key_id->value, $secret->value);
        $order = $api->order->create(array('amount' => $db->rem_price * 100, 'currency' => 'INR', 'notes' => array('userId' => $db->id, 'userName' => $db->name, 'userEmail' => $db->email, 'userContact' => $db->user_phone, 'cartId' => $db->cart_id, 'orderDate' => $db->order_date)));
        $data = array("id" => $order->id,
            ...
            "notes" => array(
                ...
                "userName" => $order->notes->userName,
                "userEmail" => $order->notes->userEmail,
                "userContact" => $order->notes->userContact,
                "cartId" => $order->notes->cartId,
                "orderDate" => $order->notes->orderDate,
            ),
            ...
            foreach ($new as $prods) {
                ...
                if (count($getrating) > 0) {
                    ...
                    $rating = DB::table('product_rating')
                        ...
                        ->where('store_id', $store_id)
                        ->avg('rating');
                    $prods->avgrating = round($rating, 2);
                ...
                foreach ($app1 as $aa) {
                    ...
                    if (count($getrating) > 0) {
                        ...
                        $rating = DB::table('product_rating')
                            ...
                            ->where('store_id', $store_id)
                            ->avg('rating');
                        $app1[$a]->avgrating = round($rating, 2);
