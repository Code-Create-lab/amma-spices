...
class OrderController extends Controller
{
    ...
    public function reject_order(Request $request)
    {
        ...
        public function viewInvoice($cart_id)
    {
        try {
            // Fetch the order with related data
            $order = Orders::with(['orderItems.variation.variation_attributes.attribute.attribute', 'address', 'user'])
                ->where('cart_id', $cart_id)
                ->firstOrFail();

            // Convert logo to base64
            $logoPath = public_path('assets/img/logo-bodhi.png');
            $logoBase64 = '';
            ...
            foreach ($order->orderItems as $item) {
                if ($item->varient_image) {
                    ...
                    if (file_exists($imagePath)) {
                        ...
                        $item->image_mime_type = $imageInfo['mime'] ?? 'image/jpeg';
                    }
                }
            }
            ...
            return $pdf->stream('Invoice-' . $order->cart_id . '.pdf');
