...
<style>
/* ═══════════════════════════════════════════════════════
   BLOG EDIT FORM — Amma's Spices Admin
...
.bcf-header__actions { display: flex; align-items: center; gap: 10px; flex-wrap: wrap; }
...
.bcf-header__preview {
    ...
    color: #e7c840;
    text-decoration: none;
    border: 1px solid rgba(231,200,64,0.3);
    background: rgba(231,200,64,0.06);
    padding: 8px 18px;
    border-radius: 8px;
    transition: background 0.2s, border-color 0.2s;
}
.bcf-header__preview:hover { background: rgba(231,200,64,0.12); border-color: #e7c840; color: #e7c840; }
...
.bcf-card__head h3 { font-size: 13.5px; font-weight: 700; color: #e0e0e0; margin: 0; letter-spacing: 0.02em; }
...
.bcf-existing-thumb img { width: 100%; height: 160px; object-fit: cover; display: block; filter: brightness(0.85); }
.bcf-existing-thumb__overlay {
...
.bcf-existing-thumb__lbl { font-size: 10px; font-weight: 700; letter-spacing: 0.08em; text-transform: uppercase; color: #e7c840; background: rgba(10,10,10,0.7); padding: 3px 8px; border-radius: 4px; }
.bcf-existing-thumb__replace { font-size: 10.5px; color: #aaa; margin-left: auto; }
...
.bcf-btn-delete {
    ...
    color: #555; border: 1px solid #1e1e1e; background: transparent;
    cursor: pointer; transition: all 0.2s; margin-top: 6px;
    text-decoration: none;
}
.bcf-btn-delete:hover { color: #e74c3c; border-color: rgba(231,76,60,0.25); background: rgba(231,76,60,0.05); }
...
.bcf-status-badge.published { background: rgba(52,168,83,0.12); border: 1px solid rgba(52,168,83,0.25); color: #4caf7d; }
.bcf-status-badge.draft     { background: rgba(251,188,5,0.1);  border: 1px solid rgba(251,188,5,0.25);  color: #f0c040; }
...
/* Last updated info strip */
.bcf-info-strip {
    display: flex; align-items: center; gap: 14px;
    padding: 12px 16px; background: #0d0d0d; border: 1px solid #1e1e1e;
    border-radius: 8px; margin-bottom: 20px; flex-wrap: wrap;
}
.bcf-info-strip__item { font-size: 11.5px; color: #555; display: flex; align-items: center; gap: 6px; }
.bcf-info-strip__item strong { color: #888; }
</style>
...
<div class="bcf-wrap">
    <div class="container-fluid">
            ...
            <div class="bcf-header__left">
                <h1>
                    ...
                    <span class="bcf-status-badge {{ $blog->status }}">
                        ...
                        @else
                            <svg width="9" height="9" viewBox="0 0 10 10" fill="none" stroke="currentColor" stroke-width="1.5"><circle cx="5" cy="5" r="4"/></svg>
                        @endif
                        {{ ucfirst($blog->status) }}
                    </span>
                </h1>
                <p>Last updated {{ $blog->updated_at->diffForHumans() }} &nbsp;·&nbsp; Created {{ $blog->created_at->format('d M Y') }}</p>
            </div>
            <div class="bcf-header__actions">
                @if($blog->status === 'published')
                    <a href="{{ route('customer.blog.show', $blog->slug) }}" target="_blank" class="bcf-header__preview">
        ...
        <form action="{{ route('admin.blog.update', $blog->id) }}" method="POST" enctype="multipart/form-data" id="bcfForm">
            @csrf
            @method('PUT')
                    ...
                    <div class="bcf-card">
                        ...
                        <div class="bcf-card__body">
                            ...
                            <div class="bcf-field">
                                ...
                                <div class="bcf-counter" id="titleCounter">{{ strlen(old('title', $blog->title)) }} / 120</div>
                            </div>
                        ...
                        <div class="bcf-card__body">
                            ...
                            <div class="bcf-field">
                                ...
                                <textarea id="excerpt" name="excerpt" class="bcf-textarea" rows="3"
                                          maxlength="200"
                                          placeholder="Short summary for listing card. Leave blank to auto-generate.">{{ old('excerpt', $blog->excerpt) }}</textarea>
                                <div class="bcf-counter" id="excerptCounter">{{ strlen(old('excerpt', $blog->excerpt ?? '')) }} / 200</div>
                            </div>

                            <div class="bcf-field">
                                <label class="bcf-label" for="content">Article Body <span>*</span></label>
                                <textarea id="content" name="content" class="bcf-textarea" rows="18"
                    ...
                    <div class="bcf-card">
                        ...
                        <div class="bcf-card__body">
                            ...
                            <div class="bcf-field">
                                ...
                                <div class="bcf-counter" id="metaTitleCounter">{{ strlen(old('meta_title', $blog->meta_title ?? '')) }} / 60</div>
                            </div>
                            ...
                            <div class="bcf-field">
                                ...
                                <div class="bcf-counter" id="metaDescCounter">{{ strlen(old('meta_description', $blog->meta_description ?? '')) }} / 160</div>
                            </div>

                            <div class="bcf-seo-lbl">Google Preview</div>
                            <div class="bcf-seo-preview">
                                <div class="bcf-seo-preview__url">{{ url('/blog/') }}/<span id="seoSlug">{{ $blog->slug }}</span></div>
                                <div class="bcf-seo-preview__title" id="seoTitle">{{ $blog->meta_title ?: $blog->title }}</div>
                                <div class="bcf-seo-preview__desc" id="seoDesc">{{ $blog->meta_description ?: ($blog->excerpt ?: 'Your meta description will appear here.') }}</div>
                            </div>
                    ...
                    <div class="bcf-card">
                        ...
                        <div class="bcf-card__body">
                            <div class="bcf-field">
                                ...
                                <div class="bcf-tags-help">Separate with commas. These appear as pills on the blog detail page.</div>
                            </div>
